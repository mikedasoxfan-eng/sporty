...
/* ------------------------------------------------------------------ */
/* Category header -- teams get ESPN logos, stats get styled badges     */
/* ------------------------------------------------------------------ */
function CategoryHeader({
  ...
  const teamAbbr = cat.teamAbbr;
  const logoUrl = teamAbbr ? teamLogos[teamAbbr] : null;
...
/* Main NFL grid page                                                  */
/* ------------------------------------------------------------------ */
export default function NFLGridPage() {
  ...
  // Fetch grid config and team logos
  useEffect(() => {
    fetch("/api/nfl-grid")
      .then((r) => r.json())
      .then(setGrid)
    ...
    // Fetch team logos from the teams API or use a static approach
    fetch("/api/nfl-teams-logos")
      .then((r) => r.json())
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/football-search?q=${encodeURIComponent(searchQuery)}`,
  ...
  const submitGuess = useCallback(
    async (player: SearchResult) => {
      ...
      try {
        const res = await fetch("/api/nfl-grid", {
          method: "POST",
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-muted">Loading today&apos;s NFL grid...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[560px] mx-auto px-4 py-6 md:py-10">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-[11px] text-muted-light mt-1 font-mono tracking-wider">
          {grid.gridId}
        </p>
      ...
      >
        ...
        <div
          className="absolute inset-0 pointer-events-none z-10 opacity-[0.015] mix-blend-overlay"
          style={{
            backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)'/%3E%3C/svg%3E")`,
          }}
        />
        <div className="absolute inset-0 pointer-events-none z-10 bg-gradient-to-br from-white/[0.04] via-transparent to-white/[0.02]" />
        ...
        <div className="grid grid-cols-[80px_1fr_1fr_1fr]">
          ...
          {grid.rows.map((row, ri) => (
            <React.Fragment key={`row-${ri}`}>
              {/* Row header */}
              <div
                className="h-[110px] md:h-[130px] flex items-center justify-center px-2
              ...
              {grid.cols.map((_, ci) => {
                ...
                if (cell.status === "correct") {
                  return (
                    <Link
                      key={`${ri}-${ci}`}
                      href={`/football/players/${cell.playerId}`}
                    ...
                    >
                      ...
                      {/* Green check */}
                      <div className="absolute top-1.5 right-1.5 w-4 h-4 rounded-full bg-green-500/90 flex items-center justify-center z-10">
                      ...
                      {/* Name */}
                      <span
                        className="absolute bottom-2 inset-x-1 text-[9px] md:text-[10px]
                  ...
                  >
                    {isActive && (
                      <div
                        ...
                        animate-pulse"
                      >
                        <span className="text-accent/50 text-lg font-light">
                          +
                        </span>
                      </div>
                    )}
        ...
        <details className="border-t border-white/[0.06]">
          <summary
            className="px-4 py-2.5 text-[10px] text-muted-light cursor-pointer
          ...
          <div className="px-4 pb-3 text-[10px] text-muted leading-relaxed space-y-1">
            <p>
              Select a cell and name an NFL player who matches{" "}
              <strong>both</strong> the row and column.
            </p>
            <p>
              Team + team: played for both. Team + stat: achieved the stat while
              playing for that team or in their career.
            </p>
      ...
      {activeCell && !gameOver && (
        <div className="mb-5 animate-[fadeIn_0.15s_ease-out]">
          <div
            className="backdrop-blur-xl bg-surface/80 border border-white/[0.08] rounded-2xl
            ...
            <div className="flex items-center gap-2 px-4 py-3 border-b border-border-light">
              <div className="flex-1 flex items-center gap-2">
                ...
                <span className="text-[10px] text-muted-light px-1.5 py-0.5 rounded bg-surface-alt font-mono">
                  {grid.rows[activeCell[0]].label} +{" "}
                  {grid.cols[activeCell[1]].label}
                </span>
              </div>
              <button
                onClick={() => {
